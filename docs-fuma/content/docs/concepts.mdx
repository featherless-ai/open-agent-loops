...
```ts
import { runAgent, maxSteps, whenToolCalled, any } from "@open-agent-os/core";
...
- **Testing** — `MockModelClient`, a scripted `ModelClient` imported from
  `@open-agent-os/core/mocks/mock-model`.

Opt-in provider battery:

- **`OpenAICompatibleModel`** — a complete `ModelClient` for any
  OpenAI-compatible endpoint, from the `@open-agent-os/core/providers/openai` subpath
