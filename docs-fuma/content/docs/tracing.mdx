...
<Callout title="Readable by default, quoting handled">
  `toCurl` pretty-prints the JSON body so the command is easy to read — single
  quotes preserve the newlines, so it stays runnable (pass `pretty: false` for a
  compact one-liner). It also escapes any embedded apostrophe as `'\''`, so
  message content like *"what's the weather"* survives the shell intact. For very
  large multi-turn bodies, prefer writing the body to a file and using
