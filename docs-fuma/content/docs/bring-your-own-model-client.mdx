...
```ts
import { runAgent, SessionMemoryStore, StreamEventType, assistantMessage } from "@open-agent-loops/agent-loop-core";
...
```ts
import { StreamEventType, ToolCallType, assistantMessage } from "@open-agent-loops/agent-loop-core";
import type { ModelClient, ModelRequest, StreamEvent, ToolSpec, ToolCall } from "@open-agent-loops/agent-loop-core";
...
<Callout title="Already OpenAI-compatible? Use the battery.">
  ...
  any of this — `OpenAICompatibleModel` from `@open-agent-loops/agent-loop-core/providers/openai` is a
