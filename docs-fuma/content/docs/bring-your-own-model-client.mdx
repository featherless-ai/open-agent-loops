...
```ts
import { runAgent, SessionMemoryStore, StreamEventType, assistantMessage } from "@open-agent-os/core";
...
```ts
import { StreamEventType, ToolCallType, assistantMessage } from "@open-agent-os/core";
import type { ModelClient, ModelRequest, StreamEvent, ToolSpec, ToolCall } from "@open-agent-os/core";
...
<Callout title="Already OpenAI-compatible? Use the battery.">
  ...
  any of this — `OpenAICompatibleModel` from `@open-agent-os/core/providers/openai` is a
