---
title: Gating tool calls
description: A short tutorial — admit, deny, or ask before tool calls run, and compose permissions with credentials.
---

Some tool calls are risky enough that a human should approve them first. This
tutorial picks up where the [multi-turn chat
...
```ts
import { withCredentials, InMemoryCredentialStore } from "agent-core";
...
// Same chat loop as above — just wrap the tool; the gate is unchanged.
await runAgent({
  model,
  memory,
  sessionId,
...
A **skill** guards the same way: its bundled tools are just tools, so wrap each
with `withCredentials` and let the same `gateToolCalls` hook admit them. (Skills
are a design sketch — `agent-core/docs/skills.md` — for now; the composition
above is what they build on.)
