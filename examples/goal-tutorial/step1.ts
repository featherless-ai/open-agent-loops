/**
 * Goal-loop tutorial — Step 1: grade a run yourself.
 *
 * One growing program, built up across examples/goal-tutorial/step1..4.ts — each
 * step is the previous file plus a few lines (highlighted in the docs). Here:
 * `runGoal` is the *outer* loop. It runs `runAgent`, hands the result to a grader
 * you write, and when the grader isn't satisfied its feedback becomes the next
 * round's prompt — until the answer passes or the round cap is hit.
 *
