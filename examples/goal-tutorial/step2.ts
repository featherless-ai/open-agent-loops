/**
 * Goal-loop tutorial — Step 2: watch each round.
 *
 * One growing program, built up across examples/goal-tutorial/step1..4.ts — each
 * step is the previous file plus a few lines (highlighted in the docs). Here: an
 * `onRound` hook prints the grader's verdict as each round settles, so you can
 * watch the loop redirect itself.
 *
...
const outcome = await runGoal({
  ...
  onRound: ({ round, grade }) => { // [!code highlight:4]
